...
import VideoAds from './VideoAds'; // Import the VideoAds component
...
const ImageConverter = () => {
  ...
  useEffect(() => {
    if (originalFile) {
      // Fake delay to simulate large image loading
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        if (webPFile) {
          ...
          triggerDownload(fileData); // Automatically download after conversion
        } else {
  ...
  // Function to calculate load time based on file size and network speed
  const calculateLoadTime = (fileSizeMB, networkSpeedMbps) => {
    const fileSizeBits = fileSizeMB * 1024 * 1024 * 8; // Convert MB to bits
    const networkSpeedBps = networkSpeedMbps * 1024 * 1024; // Convert Mbps to bits per second
    return (fileSizeBits / networkSpeedBps).toFixed(2); // Return time in seconds
  };

  // Estimate load times for both original and converted images
  const originalLoadTime = calculateLoadTime(originalFile ? originalFile.size / 1024 / 1024 : 0, 5); // Assuming 5 Mbps
  ...
  return (
    <div className="p-4 w-full max-w-5xl mx-auto">
      ...
      {isProcessing && (
        <>
          <VideoAds /> {/* Display video ads when processing */}
          <ProcessingIndicator
            messages={messages}
            duration={duration}
            fileSize={fileSize}
      ...
      {!isProcessing && renameOption && (
        <div className="flex flex-col md:flex-row justify-between mt-12 space-x-0 md:space-x-4">
          {/* Use md:order-* classes to control order based on screen size */}
